...
- $x^{(i)}$ is the $i_{th}$ training example _(ie. the $i^{th}$ row of $X$)_, alternatively $y^{(i)}$ is the $i^{th}$ target value.
- $x_{j}$ represents the $j^{th}$ feature, _(ie. the $j^{th}$ column of $X$)_.
